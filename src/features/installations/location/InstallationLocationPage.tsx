...
function LocationSummary({ id, state }: { id: string; state: LocationState }) { return <section className="installationLocationSummary"><h2>Résumé</h2><dl><dt><UserRound size={17} />Client</dt><dd>{state.client}</dd><dt><Activity size={17} />Installation</dt><dd>{id}</dd><dt><House size={17} />Type</dt><dd>{state.site}</dd><dt><MapPin size={17} />Localisation</dt><dd>{state.location}</dd><dt><Activity size={17} />Statut global</dt><dd><b className={`tone-${state.tone}`}><i />{state.headerStatus}</b></dd></dl></section> }
