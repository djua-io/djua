...
function Layout({children}:{children:React.ReactNode}){
  ...
  return <div className={'shell '+(isQuoteDetail?'quoteDetailShell':'')}><aside className={isQuoteDetail?'quoteDetailAside':''}><Link to="/dashboard" className="brand shopBrand"><i>S</i><span>Shop Victoire</span></Link><Link to="/dimensionnements/nouveau/client" className="newQuote"><I.Plus size={17}/> Nouveau devis <I.ChevronDown size={16}/></Link><nav>{nav.map(([n,p])=>{const Icon=names[n];return <NavLink to={p} key={p} className={({isActive})=>isActive?'active':''}><Icon size={18}/>{n}</NavLink>})}</nav><div className="sidebarFooter"><button type="button" className="profile" aria-label="Compte de Chris M."><span className="avatar"><I.UserRound size={18}/></span><span><b>Chris M.</b><small>Commercial</small></span><I.ChevronDown size={16}/></button><button type="button" className="help"><I.CircleHelp size={18}/><span><b>Besoin d’aide ?</b><small>Consultez notre centre d’aide</small></span></button></div></aside><main><header className={isQuoteDetail?'quoteDetailHeader':'quotesHeader '+(isRecommendation?'recommendationHeader':'')}>{isQuoteDetail?<Link to="/devis" className="quoteBack"><I.ArrowLeft size={19}/> Retour à la liste des devis</Link>:<>{isSizingFlow&&!isQuoteList&&<button className="plain headerBack" onClick={goBack}><I.ArrowLeft size={19}/></button>}<b>{headerTitle}</b>{isSizingFlow&&!isRecommendation&&<Badge text="Brouillon"/>}<span className="spacer"/><label className="globalSearch"><I.Search size={17}/><input placeholder="Rechercher un client, un devis…"/><kbd>⌘ K</kbd></label><button className="headerBell" aria-label="Notifications"><I.Bell size={19}/><i/></button></>}</header>{children}</main></div>
}
