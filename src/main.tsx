...
function ParcSolaire(){
  ...
  return <section className="parcSolarPage">
    ...
    <section className="parcOverviewGrid">
      ...
      <section className="recentAlerts"><header><div><span className="recentAlertCount"><I.BellRing size={14}/>4 à traiter</span><h2>Alertes récentes</h2></div><button>Voir tout <I.ArrowUpRight size={15}/></button></header><div className="parkAlertList">{parkAlerts.map(alert=>{const Icon=alert.tone==='critical'?I.CircleAlert:I.TriangleAlert;return <article className={'parkAlert '+alert.tone} key={alert.id}><i><Icon size={19}/></i><div className="parkAlertBody"><div className="parkAlertMeta"><span><b>{alert.id}</b><em>{alert.tag}</em></span><time><I.Clock size={12}/>{alert.time}</time></div><small><I.MapPin size={12}/>{alert.name} · {alert.place}</small><p>{alert.detail}</p></div><button className="parkAlertAction" aria-label={'Ouvrir l’alerte '+alert.id}><I.ChevronRight size={18}/></button></article>})}</div></section>
    </section>
