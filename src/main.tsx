...
function Reports(){const [tab,setTab]=useState('Vue d’ensemble');const reportsRows=[['Production faible','68'],['Consommation élevée','43'],['Communication interrompue','39'],['Tension batterie anormale','24'],['Déplacement détecté','13']];const productRows=[['Panneau solaire 450 W','Panneau solaire','412',solarPanelsProduct],['Batterie LiFePO4 5 kWh','Batterie','286',batteryProduct],['Onduleur hybride 5 kVA','Onduleur','244',inverterProduct],['Kit câblage solaire','Accessoire','198',solarCableKit],['Batterie GEL 5 kWh','Batterie','176',batteryProduct]];const anomalyIcons=[I.TriangleAlert,I.TriangleAlert,I.Link2,I.BatteryWarning,I.MapPin];return <section className="workspacePage reportsPage"><header className="workspaceHeading workspacePageHeader reportsHead"><div><h1>Rapports</h1><p>Analysez vos activités commerciales et la performance de votre parc solaire.</p></div><div className="workspaceActions"><ReportDateRangePicker/><ExportMenu title="Rapport Djúa — août 2026" fileName="rapport-djua-aout-2026" columns={['Indicateur','Valeur']} rows={reportsRows}/></div></header><nav className="reportTabs">{['Vue d’ensemble','Dimensionnement & ventes','Performance du parc','Rapports personnalisés'].map(item=><button type="button" key={item} className={tab===item?'active':''} onClick={()=>setTab(item)}>{item}</button>)}</nav><div className="reportKpis">{[{icon:I.Calculator,label:'Dimensionnements réalisés',value:'128',gain:'+ 12 %',tone:'orange'},{icon:I.FileText,label:'Devis créés',value:'86',gain:'+ 8 %',tone:'amber'},{icon:I.CircleCheck,label:'Devis acceptés',value:'42',gain:'+ 24 %',tone:'green'},{icon:I.UsersRound,label:'Installations suivies',value:'4 286',gain:'+ 5 %',tone:'purple'}].map(({icon:Icon,label,value,gain,tone})=><article key={label}><i className={tone}><Icon size={24}/></i><span><small>{label}</small><b>{value}</b><em><I.TrendingUp size={15}/>{gain}<small>vs période précédente</small></em></span></article>)}</div><div className="reportCharts"><ReportChart type="commercial"/><ReportChart type="energy"/></div><div className="reportLower"><section className="reportCard reportDistribution"><h2>Répartition des installations par statut</h2><div><div className="reportDonut"><strong>4 286<small>installations</small></strong></div><ul>{[['Actives','3 672','85,6 %','green'],['Attention','342','8,0 %','yellow'],['En panne','156','3,6 %','red'],['Hors ligne','116','2,8 %','gray']].map(([label,value,percent,tone])=><li key={label}><i className={tone}/><span>{label}</span><b>{value}</b><small>{percent}</small></li>)}</ul></div></section><section className="reportCard reportAnomalies"><header><h2>Top 5 des anomalies</h2><button type="button">Voir tout</button></header><div className="reportTableHead"><span>Type d’anomalie</span><span>Nombre</span></div>{reportsRows.map(([label,value],index)=>{const Icon=anomalyIcons[index];return <p key={label}><Icon size={16}/><span>{label}</span><b>{value}</b></p>})}</section><section className="reportCard reportProducts"><header><h2>Produits les plus utilisés</h2><button type="button">Voir tout</button></header><div className="reportProductHead"><span>Produit</span><span>Catégorie</span><span>Utilisations</span></div>{productRows.map(([name,category,uses,image])=><article key={name}><img src={image} alt=""/><b>{name}</b><span>{category}</span><strong>{uses}</strong></article>)}</section></div><footer className="reportAdvice"><I.Lightbulb size={28}/><span><h2>Conseil Djua</h2><p>12 % de plus de dimensionnements ce mois-ci. Le taux de conversion des devis a également augmenté de 24 %.</p></span><button type="button">Voir l’analyse détaillée <I.ArrowRight size={17}/></button></footer></section>}
