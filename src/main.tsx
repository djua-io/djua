...
function ProductsCatalogue(){
  const navigate=useNavigate()
  const [products,setProducts]=useState<CatalogueProduct[]>(()=>[...catalogueProducts,...store.get<CatalogueProduct[]>('djua-catalogue-products',[])])
  ...
  const toggleProductStatus=(target:CatalogueProduct)=>{setProducts(current=>current.map(product=>product===target?{...product,active:!product.active}:product));setOpenAction(null)}
...
function ProductDetail(){
  const navigate=useNavigate()
  const {id}=useParams()
  const product=[...catalogueProducts,...store.get<CatalogueProduct[]>('djua-catalogue-products',[])].find(item=>item.name===decodeURIComponent(id||''))||catalogueProducts[0]
  ...
  const categoryDetails={
    ...
    'Batteries':{metrics:[{icon:I.Battery,label:'Capacité',value:product.power},{icon:I.Zap,label:'Tension nominale',value:'48 V'},{icon:I.Settings,label:'Technologie',value:'LiFePO₄'},{icon:I.ShieldCheck,label:'Garantie',value:'5 ans'}],technical:[['Capacité',product.power],['Tension nominale','48 V'],['Technologie','LiFePO₄'],['Capacité (Ah)','100 Ah'],['Profondeur de décharge recommandée','80 %'],['Poids','45 kg']],advice:'Les produits de batteries doivent avoir des informations complètes sur la capacité, la tension et la technologie. Veillez à maintenir ces informations à jour pour maximiser leur visibilité auprès de vos clients.',usage:'42 recommandations'},
    'Onduleurs':{metrics:[{icon:I.Zap,label:'Puissance nominale',value:product.power},{icon:I.Battery,label:'Tension batterie',value:'48 V'},{icon:I.Settings,label:'Type',value:'Hybride'},{icon:I.ShieldCheck,label:'Garantie',value:'5 ans'}],technical:[['Puissance nominale',product.power],['Tension batterie','48 V'],['Type','Hybride'],['Tension de sortie AC','220 V'],['Puissance de crête','6000 W'],['Poids','18 kg']],advice:'Les onduleurs doivent avoir des informations complètes sur la puissance nominale, la tension batterie et le type. Veillez à maintenir ces informations à jour pour maximiser leur visibilité auprès de vos clients.',usage:'38 recommandations'},
