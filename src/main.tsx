...
function ProductsCatalogue(){
  const navigate=useNavigate()
  const products=useMemo(()=>[...catalogueProducts,...store.get<CatalogueProduct[]>('djua-catalogue-products',[])],[])
...
function ProductCreate(){
  ...
  const [name,setName]=useState('Panneau solaire 600 W')
  const [brand,setBrand]=useState('Orange Solar')
  const [reference,setReference]=useState('PS600')
  const [price,setPrice]=useState('320')
  const [power,setPower]=useState('600')
  const [technology,setTechnology]=useState('Monocristallin')
  const [warranty,setWarranty]=useState('10')
  const [active,setActive]=useState(true)
  const [used,setUsed]=useState(true)
  const categoryInfo={
    'Panneaux solaires':{title:'Panneau solaire',hint:'Modules photovoltaïques',icon:I.Sun,image:solarPanelsProduct},
    'Batteries':{title:'Batterie',hint:"Stockage d’énergie",icon:I.Battery,image:batteryProduct},
    'Onduleurs':{title:'Onduleur',hint:'Conversion DC / AC',icon:I.Activity,image:inverterProduct},
    'Accessoires':{title:'Accessoire',hint:'Câbles, fixations, etc.',icon:I.Box,image:solarCableKit}
  } as const
  const selection=categoryInfo[category]
  const SelectedIcon=selection.icon
  const completeness=Math.round(([name,brand,reference,price,power,technology,warranty].filter(Boolean).length/9)*100)
  const saveProduct=()=>{const product:CatalogueProduct={name:name.trim(),brand:`${brand.trim()} · ${reference.trim()}`,category,power:`${power} W`,detail:technology,price:`${price} $`,image:selection.image,active,used};store.set('djua-catalogue-products',[product,...store.get<CatalogueProduct[]>('djua-catalogue-products',[])]);navigate('/produits')}
  return <section className="workspacePage productCreatePage">
    <form onSubmit={event=>{event.preventDefault();saveProduct()}}>
      <header className="productCreateHead"><div><button type="button" className="productBack" onClick={()=>navigate('/produits')}><I.ChevronLeft size={16}/>Catalogue</button><h1>Ajouter un produit</h1><p>Ajoutez un produit qui sera utilisé par Djúa pour construire les recommandations.</p></div><div className="productCreateActions"><Button secondary onClick={()=>navigate('/produits')}>Annuler</Button><Button><I.Save size={16}/>Enregistrer le produit</Button></div></header>
      <div className="productCreateLayout">
        <main className="productCreateForm">
          <section className="productFormCard"><header><i>1</i><span><h2>Catégorie du produit</h2><p>Sélectionnez la catégorie à laquelle appartient ce produit.</p></span></header><div className="productCategoryChoices">{(Object.keys(categoryInfo) as Exclude<CatalogueCategory,'Tous'>[]).map(key=>{const info=categoryInfo[key];const Icon=info.icon;return <button type="button" className={category===key?'selected':''} key={key} onClick={()=>setCategory(key)}><i><Icon size={23}/></i><span><b>{info.title}</b><small>{info.hint}</small></span>{category===key?<I.CheckCircle2 size={18}/>:<I.Circle size={18}/>}</button>})}</div></section>
          <section className="productFormCard"><header><i>2</i><span><h2>Informations générales</h2><p>Renseignez les informations principales du produit.</p></span></header><div className="productGeneralLayout"><div className="productFields"><label>Nom du produit <b>*</b><input required value={name} onChange={event=>setName(event.target.value)}/></label><label>Marque <b>*</b><input required value={brand} onChange={event=>setBrand(event.target.value)}/></label><label>Référence / Modèle <b>*</b><input required value={reference} onChange={event=>setReference(event.target.value)}/></label><label>Prix de vente (USD) <b>*</b><span className="productUnitField"><input required inputMode="decimal" value={price} onChange={event=>setPrice(event.target.value)}/><em>$</em></span></label><label>SKU <small>(optionnel)</small><input placeholder="Ex. OS-PS600"/></label></div><aside className="productImageUpload"><b>Image du produit</b><div><img src={selection.image} alt="Aperçu du produit"/><button type="button"><I.Camera size={15}/>Changer l’image</button><small>JPG, PNG ou WebP (max. 5 Mo)</small></div></aside></div></section>
          <section className="productFormCard"><header><i>3</i><span><h2>Caractéristiques techniques</h2><p>Renseignez les caractéristiques techniques principales de ce {selection.title.toLowerCase()}.</p></span></header><div className="productTechFields"><label>Puissance nominale (W) <b>*</b><span className="productUnitField"><input required inputMode="decimal" value={power} onChange={event=>setPower(event.target.value)}/><em>W</em></span></label><label>Technologie <b>*</b><select value={technology} onChange={event=>setTechnology(event.target.value)}><option>Monocristallin</option><option>Polycristallin</option><option>Hybride</option></select></label><label>Garantie <b>*</b><span className="productUnitField"><input required inputMode="numeric" value={warranty} onChange={event=>setWarranty(event.target.value)}/><em>ans</em></span></label></div><details className="productAdvanced"><summary><I.ChevronDown size={17}/><span><b>Caractéristiques avancées</b> <small>(optionnel)</small><em>Informations complémentaires pour affiner les recommandations.</em></span></summary><div><label>Tension nominale (V)<input placeholder="41,5"/></label><label>Courant max (A)<input placeholder="14,5"/></label></div></details></section>
          <section className="productFormCard productUsageCard"><header><i>4</i><span><h2>Utilisation dans Djúa</h2><p>Définissez comment ce produit sera utilisé dans la plateforme.</p></span></header><div><label className="productUsageSwitch"><span><b>Utiliser pour les recommandations</b><small>Ce produit pourra être proposé dans les recommandations Djúa.</small></span><span className="catalogueSwitch"><input type="checkbox" checked={used} onChange={()=>setUsed(value=>!value)}/><i/></span></label><label className="productStatusField"><b>Statut du produit</b><select value={active?'Actif':'Inactif'} onChange={event=>setActive(event.target.value==='Actif')}><option>Actif</option><option>Inactif</option></select><small>Les produits inactifs ne sont pas proposés dans les recommandations.</small></label></div><p className="productFormWarning"><I.TriangleAlert size={18}/><span><b>Les produits incomplets ne seront pas utilisés dans les recommandations.</b> Assurez-vous de renseigner les informations essentielles avant d’enregistrer ce produit.</span></p></section>
        </main>
        <aside className="productCreateRail"><section className="productPreview"><header><I.Sun size={20}/><h2>Résumé du produit</h2></header><img src={selection.image} alt=""/><dl><dt>Catégorie</dt><dd>{selection.title}</dd><dt>Nom</dt><dd>{name||'—'}</dd><dt>Marque</dt><dd>{brand||'—'}</dd><dt>Référence</dt><dd>{reference||'—'}</dd><dt>Prix de vente</dt><dd>{price?`${price} $`:'—'}</dd><dt>Statut</dt><dd><span className={'catalogueStatus '+(active?'active':'inactive')}><i/>{active?'Actif':'Inactif'}</span></dd><dt>Utilisé pour les recommandations</dt><dd><span className="catalogueSwitch"><input type="checkbox" checked={used} readOnly/><i/></span></dd></dl><button type="button" onClick={()=>navigate('/produits')}><I.ExternalLink size={15}/>Voir le produit après enregistrement</button></section><section className="productCompleteness"><header><I.Lightbulb size={20}/><h2>Complétude</h2><strong>{completeness} %</strong></header><div><i><span style={{width:`${completeness}%`}}/></i></div><ul><li><I.CheckCircle2 size={16}/>Informations générales <b>5/5</b></li><li><I.CheckCircle2 size={16}/>Caractéristiques principales <b>3/3</b></li><li><I.Circle size={16}/>Caractéristiques avancées <b>0/2</b></li><li><I.CheckCircle2 size={16}/>Utilisation dans Djúa <b>2/2</b></li></ul></section><section className="productAdvice"><I.Lightbulb size={22}/><span><h2>Conseil Djúa</h2><p>Pour un {selection.title.toLowerCase()}, les informations les plus importantes sont la puissance nominale, la technologie et le prix de vente.</p><p>Seuls les produits complets sont utilisés dans les recommandations afin de garantir des résultats fiables.</p></span></section></aside>
      </div>
    </form>
  </section>
}
function App(){return <Layout><Routes><Route path="/" element={<Dashboard/>}/><Route path="/dashboard" element={<Dashboard/>}/><Route path="/dimensionnements" element={<Dimensionnements/>}/><Route path="/dimensionnements/nouveau" element={<Navigate to="/devis" replace/>}/><Route path="/dimensionnements/nouveau/client" element={<ClientForm/>}/><Route path="/dimensionnements/:id/logement" element={<DynamicHousing/>}/><Route path="/dimensionnements/:id/site" element={<DynamicHousing/>}/><Route path="/dimensionnements/:id/appareils" element={<Appliances/>}/><Route path="/dimensionnements/:id/recommandation" element={<Recommendation/>}/><Route path="/devis" element={<QuoteList/>}/><Route path="/devis/:id/edit" element={<QuoteEdit/>}/><Route path="/devis/:id/preview" element={<QuotePreview/>}/><Route path="/devis/:id/share" element={<Share/>}/><Route path="/devis/:id" element={<Tracking/>}/><Route path="/installations" element={<ParcSolaire/>}/><Route path="/installations/:id" element={<InstallationDetail/>}/><Route path="/produits" element={<ProductsCatalogue/>}/><Route path="/produits/nouveau" element={<ProductCreate/>}/><Route path="/parametres" element={<Settings/>}/>{['/clients','/interventions','/rapports'].map(x=><Route key={x} path={x} element={<Simple title={x.slice(1)[0].toUpperCase()+x.slice(2)}/>}/>)}</Routes></Layout>}
