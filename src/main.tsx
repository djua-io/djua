...
function QuoteList(){
  ...
  return <section className="page quoteListPage">
    ...
    <section className="quoteMetricGrid">
      <div><i className="orange"><I.FileText size={23}/></i><span><small>Devis en cours</small><b>24</b><em>Propositions actives</em></span></div>
      <div className="followMetric"><i className="orange"><I.Bell size={23}/></i><span><small>À relancer</small><b>6</b><em>2 relances prévues aujourd’hui</em></span></div>
    ...
    <section className="allQuotesPanel">
      ...
      <div className="quoteDataTable"><div className="quoteDataHeader"><span><input aria-label="Sélectionner tous les devis" type="checkbox"/></span><span>Devis</span><span>Client</span><span>Montant</span><span>Paiement</span><span>Statut</span><span>Prochaine action</span><span>Date <I.ArrowDown size={14}/></span><span><I.MoreHorizontal size={18}/></span></div>{visibleQuotes.map(item=><button type="button" className="quoteDataRow" key={item.id} onClick={()=>nav('/devis/'+item.id)}><span onClick={event=>event.stopPropagation()}><input aria-label={`Sélectionner ${item.id}`} type="checkbox"/></span><span><b>{item.id}</b><small>{item.date}</small></span><span><b>{item.client}</b><small>{item.type}</small></span><strong>{money(item.amount)}</strong><span>{item.payment}<small>{item.monthly}</small></span><span><em className={'quoteStatus '+item.status.toLowerCase()}>{item.status}</em></span><span className={item.tone}><b>{item.action}</b><small>{item.actionCopy}</small></span><span>{item.date}</span><span><I.MoreHorizontal size={19}/></span></button>)}</div>
...
function Tracking(){
  ...
  const history=[
    {icon:I.MessageCircle,title:'Devis envoyé par WhatsApp',date:'28 août 2026 · 11:43',copy:''},
    {icon:I.MessageCircle,title:'Relance WhatsApp',date:'1 sept. 2026 · 09:15',copy:'“Avez-vous eu l’occasion de consulter le devis ?”'},
    {icon:I.Phone,title:'Appel client',date:'3 sept. 2026 · 14:32',copy:'Intéressé — souhaite réfléchir'},
    {icon:I.CalendarDays,title:'Relance prévue',date:'6 sept. 2026 · WhatsApp',copy:''},
    {icon:I.Hourglass,title:'En attente de décision du client',date:'',copy:''}
  ];
  return <section className="quoteDetailPage"><div className="quoteDetailWorkspace"><div className="quoteDetailMain"><div className="quoteDetailTitle"><h1>Devis OE-2026-00847 <Badge text="Partagé"/></h1><p>Créé par Chris M.　•　28 août 2026 à 11:40</p></div><section className="detailMetaCard"><div className="detailClient"><i><I.UserRound size={25}/></i><span><b>Jean Kabeya</b><small>Maison individuelle<br/>Kinshasa, RDC</small></span></div><div><i><I.CalendarDays size={22}/></i><span><small>Date du devis</small><b>28 août 2026</b></span></div><div><i><I.CalendarDays size={22}/></i><span><small>Validité du devis</small><b>28 sept. 2026 (30 jours)</b></span></div><div className="detailAmount"><span><small>Montant total</small><b>3 480 $</b><em>Paiement échelonné · 24 mois</em></span></div></section><div className="detailTabs">{['Suivi du devis','Détails du devis','Documents','Activités'].map(item=><button type="button" key={item} onClick={()=>setTab(item)} className={tab===item?'active':''}>{item}</button>)}</div>{tab==='Suivi du devis'?<><section className="followWorkspace"><div className="clientFollow"><div className="followWorkspaceTitle"><i><I.MessageCircle size={23}/></i><span><h2>Relance client</h2><p><Badge text="À relancer"/> Aucune réponse depuis 4 jours</p></span></div><div className="followCallout"><p>Le client n'a pas répondu depuis 4 jours.</p><p>Une relance peut être utile pour faire avancer le projet.</p></div><div className="clientFollowActions"><Button onClick={()=>add('Relance WhatsApp envoyée à Jean Kabeya')}><I.MessageCircle size={17}/> WhatsApp</Button><Button secondary onClick={()=>add('Appel client — Intéressé, souhaite réfléchir')}><I.Phone size={17}/> Appeler</Button><Button secondary onClick={()=>add('E-mail envoyé à Jean Kabeya')}><I.Mail size={17}/> E-mail</Button><Button secondary><I.CalendarDays size={17}/> Planifier une relance</Button></div></div><form className="relancePlan" onSubmit={event=>{event.preventDefault();add(`Relance planifiée : ${relance} via ${channel}`)}}><h2>Planifier une relance</h2><p>Quand souhaitez-vous relancer Jean ?</p>{['Demain','Dans 3 jours','Dans 1 semaine','Choisir une date'].map(option=><label key={option}><input type="radio" name="relance" checked={relance===option} onChange={()=>setRelance(option)}/>{option}{option==='Choisir une date'&&<input className="relanceDate" type="date" onClick={event=>event.stopPropagation()}/>}</label>)}<div className="relanceChannel"><span>Canal</span>{['WhatsApp','Appel','E-mail'].map(option=><label key={option}><input type="radio" name="canal" checked={channel===option} onChange={()=>setChannel(option)}/>{option}</label>)}</div><label className="relanceNote">Note (facultatif)<textarea value={note} onChange={event=>setNote(event.target.value)}/></label><Button><I.CalendarDays size={16}/> Planifier</Button></form></section><section className="actionHistory"><h2>Historique des actions</h2>{history.map(({icon:Icon,title,date,copy},index)=><div className="historyItem" key={title}><i><Icon size={18}/></i><span><b>{title}</b><small>{date}</small>{copy&&<em>{copy}</em>}</span>{index<history.length-1&&<hr/>}</div>)}</section><div className="detailFootnote"><I.Lightbulb size={18}/> Le client pourra consulter le devis et nous contacter pour toute question.</div></>:<QuoteDetailAlternate tab={tab}/>}</div><aside className="quoteDetailRail"><section className="detailRailCard quoteProducts"><h2>Résumé du devis</h2><div><span>4 panneaux solaires<br/>(600 W)</span><img src={solarPanelsProduct} alt="Panneaux solaires"/></div><div><span>1 batterie (5 kWh)</span><img src={batteryProduct} alt="Batterie solaire"/></div><div><span>1 onduleur hybride (3 kVA)</span><img src={inverterProduct} alt="Onduleur hybride"/></div><p>Services et accessoires inclus</p><strong>Total du système <b>2 850 $</b></strong></section><section className="detailRailCard detailPayment"><h2>Paiement</h2><div className="paymentDetailHighlight"><i><I.CalendarClock size={23}/></i><span><b>Paiement échelonné</b><small>24 mois · 145 $ / mois</small></span></div><dl><div><dt>Premier paiement (aujourd’hui)</dt><dd>145 $</dd></div><div><dt>Puis 23 mensualités de</dt><dd>145 $ / mois</dd></div><div><dt>Prochaine échéance</dt><dd>28 sept. 2026</dd></div></dl><hr/><dl><div><dt>Coût du paiement échelonné</dt><dd>630 $</dd></div></dl><strong>Total à payer sur 24 mois <b>3 480 $</b></strong></section><section className="detailRailCard clientInformation"><h2>Informations client</h2><p><I.Phone size={17}/><span>+243 81 234 5678</span></p><p><I.Mail size={17}/><span>jean.kabeya@email.com</span></p><p><I.Home size={17}/><span>Avenue de la Révolution, Gombe<br/>Kinshasa, RDC</span></p></section></aside></div></section>
}
function QuoteDetailAlternate({tab}:{tab:string}){
  if(tab==='Détails du devis')return <section className="detailAlternate detailSizingRecommendation"><div className="detailSizingHeading"><div><h2>Système recommandé</h2><p>Une solution solaire fiable, dimensionnée pour les usages de Jean Kabeya.</p></div><span><I.SolarPanel size={17}/> 2.4 kWc · 5 kWh</span></div><div className="detailSizingContent"><img src={solarKit} alt="Système solaire recommandé"/><div className="detailBenefitGrid"><div><i className="green"><I.ShieldCheck size={20}/></i><b>Énergie fiable</b><small>Pour les appareils essentiels</small></div><div><i className="orange"><I.BatteryCharging size={20}/></i><b>Autonomie</b><small>≈ 1 jour sans soleil</small></div><div><i className="blue"><I.TrendingUp size={20}/></i><b>Évolutif</b><small>Ajoutez des équipements plus tard</small></div><div><i className="purple"><I.BadgeCheck size={20}/></i><b>Garantie</b><small>Jusqu’à 10 ans</small></div></div></div><div className="detailEquipmentRecap"><div><img src={solarPanelsProduct} alt="Panneaux solaires"/><span><b>4 panneaux solaires</b><small>600 W · Monocristallin</small></span></div><div><img src={batteryProduct} alt="Batterie solaire"/><span><b>1 batterie LiFePO₄</b><small>5 kWh · Énergie utile 4 kWh</small></span></div><div><img src={inverterProduct} alt="Onduleur hybride"/><span><b>1 onduleur hybride</b><small>3 kVA · 48 V</small></span></div></div></section>;
