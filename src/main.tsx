...
const parkInstallations=[
  {id:'INS-00482',client:'Jean Kabeya',site:'Maison individuelle',location:'Gombe',production:'Normale',consumption:'Normale',communication:'Hors ligne',global:'Critique',last:'Il y a 7 h',status:'critical' as ParkStatus},
  {id:'INS-00841',client:'Kivu Market SARL',site:'Commerce',location:'Gombe',production:'Plus faible',consumption:'Normale',communication:'En ligne',global:'À surveiller',last:'Il y a 15 min',status:'watch' as ParkStatus},
  {id:'INS-00912',client:'Sarah Ilunga',site:'Maison individuelle',location:'Ngaliema',production:'Normale',consumption:'Plus élevée',communication:'En ligne',global:'À surveiller',last:'Il y a 1 h',status:'watch' as ParkStatus},
  {id:'INS-01024',client:'École La Source',site:'École',location:'Kintambo',production:'Normale',consumption:'Normale',communication:'En ligne',global:'Critique',last:'Il y a 2 h',status:'critical' as ParkStatus},
  {id:'INS-01108',client:'Martin Paluku',site:'Maison individuelle',location:'Limete',production:'Normale',consumption:'Plus faible',communication:'En ligne',global:'À surveiller',last:'Il y a 4 h',status:'watch' as ParkStatus}
]
const parkAlerts=[
...
function ParkPill({text,kind}:{text:string,kind:'normal'|'watch'|'critical'|'offline'|'high'|'low'|'online'}){
  const Icon=kind==='normal'?I.Sun:kind==='online'?I.Wifi:kind==='offline'?I.Radio:kind==='critical'?I.CircleAlert:kind==='high'?I.ArrowUp:kind==='low'?I.ArrowDown:I.TriangleAlert
...
function ParcSolaire(){
  ...
  return <section className="parcSolarPage">
    ...
    <section className="parcOverviewGrid">
      <section className="parkMapCard"><header className="parkMapHeader"><h2>Carte du parc</h2><div className="parkMapFilters"><label><I.Search size={16}/><input value={mapQuery} onChange={event=>setMapQuery(event.target.value)} placeholder="Rechercher une zone, une ville…"/></label><select aria-label="Ville"><option>Kinshasa</option></select><select aria-label="Statut" value={statusFilter} onChange={event=>setStatusFilter(event.target.value)}><option>Tous les statuts</option><option>À surveiller</option><option>Critique</option></select><button aria-label="Agrandir la carte"><I.Maximize2 size={17}/></button></div></header><div className="parkMapCanvas" aria-label={mapQuery?`Carte du parc, recherche ${mapQuery}`:'Carte des installations à Kinshasa'}><div className="parkMapRiver"/><span className="parkMapWaterLabel">Fleuve Congo</span><b className="parkMapCity">Kinshasa</b><span className="parkMapPlace ngaliema">Ngaliema</span><span className="parkMapPlace limete">Limete</span><span className="parkMapPlace masina">Masina</span><div className="parkMapLegend"><span><i className="normal"/>En bon état <b>4 041</b></span><span><i className="watch"/>À surveiller <b>187</b></span><span><i className="critical"/>Critique <b>58</b></span><span><i className="muted"/>Données indisponibles <b>43</b></span></div>{parkMarkers.map(([left,top,tone],index)=><i className={'parkMapMarker '+tone} style={{left,top}} key={index}/>) }<div className="parkMapZoom"><button aria-label="Zoom avant"><I.Plus size={18}/></button><button aria-label="Zoom arrière"><I.Minus size={18}/></button><button aria-label="Centrer la carte"><I.Crosshair size={17}/></button></div><small className="parkMapScale">5 km</small></div></section>
      <section className="recentAlerts"><header><h2>Alertes récentes</h2><button>Voir tout</button></header><div>{parkAlerts.map(alert=>{const Icon=alert.tone==='critical'?I.CircleAlert:I.TriangleAlert;return <article className="parkAlert" key={alert.id}><i className={alert.tone}><Icon size={18}/></i><span><b>{alert.id}</b><em className={alert.tone}>{alert.tag}</em><small>{alert.name} · {alert.place}</small><p>{alert.detail}</p></span><time>{alert.time}</time><I.ChevronRight size={18}/></article>})}</div></section>
    </section>
    <section className="parcInstallationsPanel"><header><h2>Toutes les installations <span>(4 286)</span></h2><div><label className="parcTableSearch"><I.Search size={16}/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Rechercher une installation…"/></label><button className="parcOutlineButton"><I.SlidersHorizontal size={16}/>Filtres</button><button className="parcOutlineButton"><I.Download size={16}/>Exporter</button></div></header><div className="parcTableScroll"><div className="parcInstallTable"><div className="parcInstallHead"><span><input aria-label="Sélectionner toutes les installations" type="checkbox"/></span><span>Installation</span><span>Client / Site</span><span>Localisation</span><span>Production<br/>vs. habitude</span><span>Consommation<br/>vs. habitude</span><span>Communication</span><span>État global</span><span>Dernière donnée</span><span>Actions</span></div>{filtered.map(item=><div className="parcInstallRow" key={item.id}><span><input aria-label={'Sélectionner '+item.id} type="checkbox"/></span><b>{item.id}</b><span><b>{item.client}</b><small>{item.site}</small></span><span>{item.location}</span><span><ParkPill text={item.production} kind={item.production==='Normale'?'normal':'low'}/></span><span><ParkPill text={item.consumption} kind={item.consumption==='Normale'?'normal':'high'}/></span><span><ParkPill text={item.communication} kind={item.communication==='En ligne'?'online':'offline'}/></span><span><ParkPill text={item.global} kind={item.status}/></span><span>{item.last}</span><button className="parkMoreButton" aria-label={'Actions pour '+item.id}><I.Ellipsis size={19}/></button></div>)}</div></div><footer><span>Affichage de {filtered.length} sur 4 286 installations</span><nav aria-label="Pagination"><button><I.ChevronLeft size={17}/></button><button className="current">1</button><button>2</button><button>3</button><button>4</button><button>5</button><span>…</span><button>858</button><button><I.ChevronRight size={17}/></button></nav></footer></section>
  </section>
}
