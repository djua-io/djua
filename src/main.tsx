...
function ParcSolaire(){
  ...
  const filtered=parkInstallations.filter(item=>`${item.id} ${item.client} ${item.site} ${item.location}`.toLowerCase().includes(query.toLowerCase()))
...
function InstallationDetail(){
  ...
  const moveCalendarMonth=(direction:number)=>setCalendarMonth(value=>{const next=dateFromKey(value);next.setMonth(next.getMonth()+direction);return dateKey(new Date(next.getFullYear(),next.getMonth(),1))})
  ...
  const shiftChartPeriod=(direction:number)=>{setChartStartDate(value=>addDays(value,direction*chartRangeDays));setChartEndDate(value=>addDays(value,direction*chartRangeDays))}
  ...
  return <section className="installationDetailPage">
    ...
    <section className="installationDetailGrid">
      ...
      <aside className="installationRail">
        <section className="installationCard installationSummary"><header><h2>Résumé installation</h2></header><dl><dt><I.UserRound size={15}/>Client</dt><dd>{installation.client}</dd><dt><I.House size={15}/>N° installation</dt><dd>{installation.id}</dd><dt><I.SolarPanel size={15}/>Type</dt><dd>{installation.site}</dd><dt><I.MapPin size={15}/>Localisation</dt><dd>{installation.location}, Kinshasa</dd><dt><I.Sparkles size={15}/>Statut global</dt><dd><ParkPill text="En ligne" kind="online"/><ParkPill text={installation.global} kind={installation.status}/></dd><dt><I.CalendarDays size={15}/>Dernière donnée</dt><dd>{installation.last}</dd></dl></section>
