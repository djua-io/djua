  ...
  return <section className="installationDetailPage">
    ...
    <section className="installationDetailGrid">
      <div className="installationMainColumn">
        ...
        <section className="installationCard technicalCard"><header><h2><I.Settings2 size={19}/>Mesures techniques</h2><button>Tout afficher<I.ChevronUp size={16}/></button></header><div>{[['Solaire','42,6 V','8,4 A','358 W'],['Batterie','25,4 V','3,2 A','En décharge'],['Sortie AC','221 V','3,5 A','774 W'],['GPS / GSM','Position conforme','06:18','Dernière transmission']].map(([title,a,b,c])=><article key={title}><b>{title}</b><span><strong>{a}</strong><strong>{b}</strong><strong>{c}</strong></span></article>)}</div></section>
      </div>
