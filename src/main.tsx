...
const reportSeries={
  commercial:[{color:'#ff5a00',label:'Dimensionnements',points:'0,128 32,116 60,111 90,98 118,105 148,92 180,95 210,76 240,84 268,69 302,77 330,58 360,62 390,50 422,53 452,38 480,42 510,28 540,35 570,24 610,33 640,28'},{color:'#0878f9',label:'Devis créés',points:'0,140 32,132 60,133 90,120 118,125 148,116 180,121 210,109 240,118 268,105 302,106 330,94 360,96 390,82 422,90 452,80 480,77 510,68 540,79 570,65 610,75 640,69'},{color:'#0aac61',label:'Devis acceptés',points:'0,156 32,151 60,153 90,145 118,150 148,143 180,148 210,137 240,141 268,130 302,133 330,123 360,125 390,112 422,119 452,106 480,115 510,108 540,97 570,109 610,100 640,94'}],
  energy:[{color:'#ff5a00',label:'Production (kWh)',points:'0,70 32,82 60,44 90,76 118,90 148,65 180,38 210,72 240,81 268,59 302,76 330,51 360,66 390,56 422,23 452,25 480,18 510,64 540,29 570,58 610,24 640,60'},{color:'#0878f9',label:'Consommation (kWh)',points:'0,129 32,121 60,130 90,112 118,125 148,117 180,135 210,103 240,114 268,107 302,121 330,108 360,123 390,105 422,114 452,106 480,102 510,130 540,112 570,120 610,100 640,118'}]
} as const
function ReportChart({type}:{type:'commercial'|'energy'}){const info=type==='commercial'?{title:'Activité commerciale',sub:'Évolution des dimensionnements, devis et devis acceptés.',ticks:['0','20','40','60','80']}:{title:'Production et consommation',sub:"Énergie mesurée sur l'ensemble du parc.",ticks:['0','500','1 000','1 500','2 000']};return <section className="reportCard reportChart"><header><span><h2>{info.title}</h2><p>{info.sub}</p></span><button type="button">Par jour <I.ChevronDown size={15}/></button></header><div className="reportPlot"><div className="reportYAxis">{info.ticks.slice().reverse().map(tick=><span key={tick}>{tick}</span>)}</div><svg viewBox="0 0 640 164" preserveAspectRatio="none" aria-label={info.title}><g className="chartGrid">{[0,41,82,123,163].map(y=><line key={y} x1="0" x2="640" y1={y} y2={y}/>)}{[0,80,160,240,320,400,480,560,640].map(x=><line key={x} x1={x} x2={x} y1="0" y2="164"/>)}</g>{reportSeries[type].map(series=><polyline key={series.label} points={series.points} fill="none" stroke={series.color} strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"/>)}</svg></div><div className="reportXAxis"><span>1 août</span><span>5 août</span><span>10 août</span><span>15 août</span><span>20 août</span><span>25 août</span><span>31 août</span></div><footer>{reportSeries[type].map(series=><span key={series.label}><i style={{background:series.color}}/>{series.label}</span>)}</footer></section>}
function ReportDateRangePicker(){const pickerRef=useRef<HTMLDivElement>(null);const [open,setOpen]=useState(false);const [start,setStart]=useState('2026-08-01');const [end,setEnd]=useState('2026-08-31');const [draftStart,setDraftStart]=useState(start);const [draftEnd,setDraftEnd]=useState(end);const [calendarMonth,setCalendarMonth]=useState('2026-08-01');const dateFromKey=(value:string)=>new Date(value+'T12:00:00');const dateKey=(value:Date)=>`${value.getFullYear()}-${String(value.getMonth()+1).padStart(2,'0')}-${String(value.getDate()).padStart(2,'0')}`;const addDays=(value:string,days:number)=>{const next=dateFromKey(value);next.setDate(next.getDate()+days);return dateKey(next)};const monthDays=(month:Date):Array<string|null>=>[...Array((new Date(month.getFullYear(),month.getMonth(),1).getDay()+6)%7).fill(null),...Array.from({length:new Date(month.getFullYear(),month.getMonth()+1,0).getDate()},(_,index)=>dateKey(new Date(month.getFullYear(),month.getMonth(),index+1)))];useEffect(()=>{if(!open)return;const close=(event:MouseEvent)=>{if(!pickerRef.current?.contains(event.target as Node))setOpen(false)};document.addEventListener('mousedown',close);return()=>document.removeEventListener('mousedown',close)},[open]);const format=(value:string,year=true)=>new Intl.DateTimeFormat('fr-FR',year?{day:'numeric',month:'short',year:'numeric'}:{day:'numeric',month:'short'}).format(dateFromKey(value));const rangeDays=Math.round((dateFromKey(end).getTime()-dateFromKey(start).getTime())/86400000)+1;const openPicker=()=>{setDraftStart(start);setDraftEnd(end);setCalendarMonth(start.slice(0,7)+'-01');setOpen(true)};const moveCalendarMonth=(direction:number)=>setCalendarMonth(value=>{const next=dateFromKey(value);next.setMonth(next.getMonth()+direction);return dateKey(new Date(next.getFullYear(),next.getMonth(),1))});const choose=(value:string)=>{if(!draftStart||draftEnd){setDraftStart(value);setDraftEnd('');return}if(value<draftStart){setDraftStart(value);return}setDraftEnd(value)};const calendarMonths=useMemo(()=>{const first=dateFromKey(calendarMonth);const second=new Date(first);second.setMonth(second.getMonth()+1);return [first,second]},[calendarMonth]);const shiftRange=(direction:number)=>{setStart(value=>addDays(value,direction*rangeDays));setEnd(value=>addDays(value,direction*rangeDays))};return <div className="chartDateControl reportDateControl" ref={pickerRef}><button className="chartDateShift" type="button" aria-label="Période précédente" onClick={()=>shiftRange(-1)}><I.ChevronLeft size={16}/></button><button className="chartDateDisplay reportDate" type="button" aria-haspopup="dialog" aria-expanded={open} onClick={openPicker}><I.CalendarDays size={16}/>{format(start,false)} – {format(end)} </button><button className="chartDateShift" type="button" aria-label="Période suivante" onClick={()=>shiftRange(1)}><I.ChevronRight size={16}/></button>{open&&<div className="chartDatePicker reportDatePicker" role="dialog" aria-label="Choisir une période"><div className="chartPickerHeader"><div><b>Choisir une période</b><small>Sélectionnez une date de début et de fin</small></div><div><button type="button" aria-label="Mois précédent" onClick={()=>moveCalendarMonth(-1)}><I.ChevronLeft size={16}/></button><button type="button" aria-label="Mois suivant" onClick={()=>moveCalendarMonth(1)}><I.ChevronRight size={16}/></button></div></div><div className="chartCalendarMonths">{calendarMonths.map(month=><section key={month.toISOString()}><h3>{new Intl.DateTimeFormat('fr-FR',{month:'long',year:'numeric'}).format(month)}</h3><div className="chartWeekdays">{['Lun','Mar','Mer','Jeu','Ven','Sam','Dim'].map(day=><span key={day}>{day}</span>)}</div><div className="chartCalendarDays">{monthDays(month).map((date,index)=>date?<button type="button" key={date} className={[date===draftStart&&'start',date===draftEnd&&'end',draftStart&&draftEnd&&date>draftStart&&date<draftEnd&&'inRange'].filter(Boolean).join(' ')} onClick={()=>choose(date)}>{Number(date.slice(-2))}</button>:<span key={'empty-'+index}/>)}</div></section>)}</div><div className="chartRangeSummary"><I.CalendarRange size={15}/>{draftStart&&draftEnd?`${format(draftStart,false)} – ${format(draftEnd)}`:'Sélectionnez la date de fin'}</div><footer><button type="button" onClick={()=>setOpen(false)}>Annuler</button><button type="button" className="apply" disabled={!draftStart||!draftEnd} onClick={()=>{setStart(draftStart);setEnd(draftEnd);setOpen(false)}}>Appliquer</button></footer></div>}</div>}
function Reports(){
  const [tab,setTab]=useState('Vue d’ensemble')
  const [province,setProvince]=useState('Toutes les provinces')
  const reportsRows=[['Production faible','68'],['Consommation élevée','43'],['Communication interrompue','39'],['Tension batterie anormale','24'],['Déplacement détecté','13']]
  const productRows=[['Panneau solaire 450 W','Panneau solaire','412',solarPanelsProduct],['Batterie LiFePO4 5 kWh','Batterie','286',batteryProduct],['Onduleur hybride 5 kVA','Onduleur','244',inverterProduct],['Kit câblage solaire','Accessoire','198',solarCableKit],['Batterie GEL 5 kWh','Batterie','176',batteryProduct]]
  const anomalyIcons=[I.TriangleAlert,I.TriangleAlert,I.Link2,I.BatteryWarning,I.MapPin]
  const provinceShare=province==='Toutes les provinces'?1:province==='Kinshasa'?.28:province==='Haut-Katanga'?.12:province==='Nord-Kivu'?.1:.06
  const count=(value:number)=>Math.max(1,Math.round(value*provinceShare)).toLocaleString('fr-FR')
  const reportKpis=[
    {icon:I.FileText,label:'Devis créés',value:count(86),gain:'+ 8 %',tone:'amber',trend:'up'},
    {icon:I.CircleCheck,label:'Devis acceptés',value:count(42),gain:'+ 24 %',tone:'green',trend:'up'},
    {icon:I.UsersRound,label:'Installations suivies',value:count(4286),gain:'+ 5 %',tone:'purple',trend:'up'},
    {icon:I.TriangleAlert,label:'Problème signalé (dans le parc)',value:count(126),gain:'- 8 %',tone:'orange',trend:'down'}
  ]
  return <section className="workspacePage reportsPage"><header className="workspaceHeading workspacePageHeader reportsHead"><div><h1>Rapports</h1><p>Analysez vos activités commerciales et la performance de votre parc solaire.</p></div><div className="workspaceActions"><ReportDateRangePicker/><label className="reportProvinceFilter"><I.MapPin size={16}/><select aria-label="Filtrer les rapports par province" value={province} onChange={event=>setProvince(event.target.value)}><option>Toutes les provinces</option>{drcProvinces.map(item=><option key={item.name}>{item.name}</option>)}</select></label><ExportMenu title={'Rapport Djúa — '+province} fileName={'rapport-djua-'+province.toLowerCase().replace(/ /g,'-')} columns={['Indicateur','Valeur']} rows={reportsRows}/></div></header><nav className="reportTabs">{['Vue d’ensemble','Dimensionnement & ventes','Performance du parc','Rapports personnalisés'].map(item=><button type="button" key={item} className={tab===item?'active':''} onClick={()=>setTab(item)}>{item}</button>)}</nav><div className="reportKpis">{reportKpis.map(({icon:Icon,label,value,gain,tone,trend})=><article key={label}><i className={tone}><Icon size={24}/></i><span><small>{label}</small><b>{value}</b><em>{trend==='down'?<I.TrendingDown size={15}/>:<I.TrendingUp size={15}/>}{gain}<small>vs période précédente</small></em></span></article>)}</div><div className="reportCharts"><ReportChart type="commercial"/><ReportChart type="energy"/></div><div className="reportLower"><section className="reportCard reportDistribution"><h2>Répartition des installations par statut</h2><div><div className="reportDonut"><strong>{count(4286)}<small>installations</small></strong></div><ul>{[['Actives',count(3672),'85,6 %','green'],['Attention',count(342),'8,0 %','yellow'],['En panne',count(156),'3,6 %','red'],['Hors ligne',count(116),'2,8 %','gray']].map(([label,value,percent,tone])=><li key={label}><i className={tone}/><span>{label}</span><b>{value}</b><small>{percent}</small></li>)}</ul></div></section><section className="reportCard reportAnomalies"><header><h2>Top 5 des anomalies</h2><button type="button">Voir tout</button></header><div className="reportTableHead"><span>Type d’anomalie</span><span>Nombre</span></div>{reportsRows.map(([label,value],index)=>{const Icon=anomalyIcons[index];return <p key={label}><Icon size={16}/><span>{label}</span><b>{count(Number(value))}</b></p>})}</section><section className="reportCard reportProducts"><header><h2>Produits les plus utilisés</h2><button type="button">Voir tout</button></header><div className="reportProductHead"><span>Produit</span><span>Catégorie</span><span>Utilisations</span></div>{productRows.map(([name,category,uses,image])=><article key={name}><img src={image} alt=""/><b>{name}</b><span>{category}</span><strong>{count(Number(uses))}</strong></article>)}</section></div><footer className="reportAdvice"><I.Lightbulb size={28}/><span><h2>Conseil Djua</h2><p>{province==='Toutes les provinces'?'12 % de plus de dimensionnements ce mois-ci.':`Données filtrées pour la province de ${province}.`} Le taux de conversion des devis a également augmenté de 24 %.</p></span><button type="button">Voir l’analyse détaillée <I.ArrowRight size={17}/></button></footer></section>}
