...
function ReportDateRangePicker(){const pickerRef=useRef<HTMLDivElement>(null);const [open,setOpen]=useState(false);const [start,setStart]=useState('2026-08-01');const [end,setEnd]=useState('2026-08-31');const [draftStart,setDraftStart]=useState(start);const [draftEnd,setDraftEnd]=useState(end);const monthDays=(month:Date):Array<string|null>=>[...Array((new Date(month.getFullYear(),month.getMonth(),1).getDay()+6)%7).fill(null),...Array.from({length:new Date(month.getFullYear(),month.getMonth()+1,0).getDate()},(_,index)=>`${month.getFullYear()}-${String(month.getMonth()+1).padStart(2,'0')}-${String(index+1).padStart(2,'0')}`)];useEffect(()=>{if(!open)return;const close=(event:MouseEvent)=>{if(!pickerRef.current?.contains(event.target as Node))setOpen(false)};document.addEventListener('mousedown',close);return()=>document.removeEventListener('mousedown',close)},[open]);const format=(value:string,year=true)=>new Intl.DateTimeFormat('fr-FR',year?{day:'numeric',month:'long',year:'numeric'}:{day:'numeric',month:'long'}).format(new Date(value+'T12:00:00'));const choose=(value:string)=>{if(!draftStart||draftEnd){setDraftStart(value);setDraftEnd('');return}if(value<draftStart){setDraftStart(value);return}setDraftEnd(value)};return <div className="reportDateControl" ref={pickerRef}><button type="button" className="workspaceAction reportDate" aria-haspopup="dialog" aria-expanded={open} onClick={()=>{setDraftStart(start);setDraftEnd(end);setOpen(true)}}><I.CalendarDays size={17}/>{format(start,false)} – {format(end)}<I.ChevronDown size={15}/></button>{open&&<div className="chartDatePicker reportDatePicker" role="dialog" aria-label="Choisir une période"><div className="chartPickerHeader"><div><b>Choisir une période</b><small>Sélectionnez une date de début et de fin</small></div><button type="button" aria-label="Fermer" onClick={()=>setOpen(false)}><I.X size={17}/></button></div><div className="chartCalendarMonths">{[new Date(2026,7,1),new Date(2026,8,1)].map(month=><section key={month.toISOString()}><h3>{new Intl.DateTimeFormat('fr-FR',{month:'long',year:'numeric'}).format(month)}</h3><div className="chartWeekdays">{['Lun','Mar','Mer','Jeu','Ven','Sam','Dim'].map(day=><span key={day}>{day}</span>)}</div><div className="chartCalendarDays">{monthDays(month).map((date,index)=>date?<button type="button" key={date} className={[date===draftStart&&'start',date===draftEnd&&'end',draftStart&&draftEnd&&date>draftStart&&date<draftEnd&&'inRange'].filter(Boolean).join(' ')} onClick={()=>choose(date)}>{Number(date.slice(-2))}</button>:<span key={'empty-'+index}/>)}</div></section>)}</div><div className="chartRangeSummary"><I.CalendarRange size={15}/>{draftStart&&draftEnd?`${format(draftStart,false)} – ${format(draftEnd)}`:'Sélectionnez la date de fin'}</div><footer><button type="button" onClick={()=>setOpen(false)}>Annuler</button><button type="button" className="apply" disabled={!draftStart||!draftEnd} onClick={()=>{setStart(draftStart);setEnd(draftEnd);setOpen(false)}}>Appliquer</button></footer></div>}</div>}
